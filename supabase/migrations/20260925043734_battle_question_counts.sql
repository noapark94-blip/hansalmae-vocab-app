-- Preserve existing 10-question rooms while accepting the new sizes.
alter table public.word_battles drop constraint word_battles_questions_check;
alter table public.word_battles add constraint word_battles_questions_check
 check (jsonb_array_length(questions) in (10,20,30)
 and coalesce((settings->>'count')::integer,10)=jsonb_array_length(questions));
