(function(){
  ...
  function syncToolbar(){var b=pageBody();if(!b)return;var checks=getChecks();if(!checks.length)return;var selected=checks.filter(function(c){return c.checked;}).length;var count=b.querySelector('#hsmSchoolSelectedCount');if(count&&count.textContent!==selected+'개 선택')count.textContent=selected+'개 선택';var save=b.querySelector('#hsmSchoolAddPersonal');if(save)save.hidden=selected===0;var toggle=b.querySelector('#hsmSchoolSelectAll');if(toggle){var all=selected===checks.length;var label=all?'전체 해제':'전체 선택';if(toggle.textContent!==label)toggle.textContent=label;toggle.setAttribute('aria-pressed',all?'true':'false');}}
