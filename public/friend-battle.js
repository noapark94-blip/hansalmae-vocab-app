...
(function(){
 ...
 function renderGame(){
  ...
  let html='<div class="fb-section-head"><h2 class="fb-game-title">WORD BATTLE</h2>'+(live()?'<div class="fb-game-tools">'+(window.HSMBattleChat?.controls(g)||'')+button('나가기','exit','','fb-text')+'</div>':'')+'</div><p class="fb-sub">'+summary(g.settings)+'</p><div id="fbConnection" class="fb-connection" role="status"></div>'+(g.status==='finished'?resultHeading(g):'')+(['invited','ready','playing','finished'].includes(g.status)?scoreboard(g):'')+(['invited','ready'].includes(g.status)?'<div class="bc-slot"></div>':'');
