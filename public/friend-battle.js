...
(function(){
 ...
 function summary(s){return esc(s.title)+(s.kind==='school'?'':' · Day '+s.start+'–'+s.end)+' · '+esc(modes[s.mode]||'')+' · 10문제';}
 ...
 function renderGame(){
  ...
  if(g.status==='invited')html+='<div class="fb-wait"><span class="fb-orbit">'+icon+'</span><h3>'+(mine?'친구의 수락을 기다려요':'친구가 대전을 신청했어요')+'</h3><p>한 문제에 20초 · 총 10문제<br>서로 준비되면 함께 시작해요.</p></div>'+(mine?'<p class="fb-note">2분 안에 수락하지 않으면 신청이 종료돼요.</p>':'<div class="fb-two">'+button('다음에 할게요','decline',g.id)+button('함께 대전하기','accept',g.id,'fb-primary')+'</div>');
   ...
   const q=g.question;
   html+='<div class="fb-progress"><i style="width:'+g.round*10+'%"></i></div><div class="fb-round"><span>'+String(g.round+1).padStart(2,'0')+' / 10</span><span id="fbTimer" aria-live="off"></span></div>';
   ...
   html+='<div class="fb-result"><span class="fb-result-badge">'+(!g.winner?'좋은 승부였어요':g.winner===g.me?'이번 대전의 주인공':'함께 한 뼘 성장했어요')+'</span><h3>'+(!g.winner?'무승부':g.winner===g.me?'멋진 승리!':'다음 판에 도전해요')+'</h3>'+(g.reason?'<p>'+esc(g.reason)+'</p>':'')+'<div class="fb-result-stats"><div><small>내 정답</small><strong>'+score+'<em> / 10</em></strong></div><div><small>정답 풀이 시간</small><strong>'+(ms/1000).toFixed(1)+'<em>초</em></strong></div></div></div><div class="fb-two">'+button('친구 목록','exit')+button('한 판 더','rematch','','fb-primary')+'</div><details class="fb-review" '+(wrong.length?'open':'')+'><summary>다시 익힐 단어 <span>'+wrong.length+'개</span></summary>'+wrong.map(w=>'<article><strong>'+esc(w.word)+'</strong><p>'+esc(w.meaning)+'</p>'+(w.example?'<div>'+esc(w.example)+(w.translation?'<small>'+esc(w.translation)+'</small>':'')+'</div>':'')+'</article>').join('')+'</details>';
 ...
 async function challenge(id){const items=await api('catalog',{target:id});if(!items.length)throw Error('함께 이용할 수 있는 단어장이 없어요.');const p=home?.friends?.find(f=>f?.id===id)||(game&&(game.host.id===id?game.host:game.guest));sheet('대전 신청', '<p class="fb-sub">'+esc(p?.name||'친구')+' 님과 10문제 한 판</p><form id="fbChallenge"><label>단어장<select id="fbSource">'+items.map(x=>'<option value="'+esc(x.id)+'">'+esc(x.title)+'</option>').join('')+'</select></label><div id="fbRange" class="fb-two"><label>시작 Day<select id="fbStart"></select></label><label>마지막 Day<select id="fbEnd"></select></label></div><label>문제 유형<select id="fbMode"><option value="mixed">영한·한영 혼합</option><option value="engToKor">영어 → 한글</option><option value="korToEng">한글 → 영어</option></select></label><p class="fb-note">정답 수 우선 · 동점이면 정답 풀이 시간 비교<br>친구가 수락하면 함께 준비할 수 있어요.</p>'+button('대전 신청 보내기','invite',id,'fb-primary fb-wide')+'</form>');
 ...
 async function handle(e){const el=e.target.closest('[data-action]');if(!el)return;e.preventDefault();if(busy)return;const a=el.dataset.action,id=el.dataset.id;
  if(a==='close'){closeModal();return;}busy=true;revision++;el.disabled=true;
  try{
   ...
   else if(a==='invite'){const g=await api('invite',{target:id,source:$('fbSource').value,start:Number($('fbStart').value),end:Number($('fbEnd').value),mode:$('fbMode').value});closeModal();stamp(g);window.scrollTo(0,0);}
 ...
 function init(){if(root)return;if(!$('mainApp')||!$('myPageScreen'))return;root=document.createElement('section');root.id='friendBattleScreen';root.className='card hidden fb-root';$('mainApp').appendChild(root);root.addEventListener('click',handle);root.addEventListener('submit',e=>{e.preventDefault();e.target.querySelector('[data-action="search"]')?.click();});
  const shortcut=document.createElement('button');shortcut.className='learning-shortcut';shortcut.id='fbShortcut';shortcut.type='button';shortcut.innerHTML='<div class="shortcut-title icon-title"><span class="section-icon">'+icon+'</span><span>친구와 단어 대전</span></div><div class="shortcut-description">함께 풀고, 함께 성장해요.</div><div class="shortcut-count" id="fbShortcutCount">친구와 함께, 10문제 한 판</div>';shortcut.addEventListener('click',open);$('myPageScreen').querySelector('.learning-shortcuts').appendChild(shortcut);
