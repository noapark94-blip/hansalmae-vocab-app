begin;
do $$
declare a uuid:=gen_random_uuid();b uuid:=gen_random_uuid();c uuid:=gen_random_uuid();room uuid;r jsonb;q jsonb;qs jsonb;i integer;count_questions integer;blocked boolean;
begin
 ...
 foreach count_questions in array array[10,20,30] loop
 select jsonb_agg(jsonb_build_object('prompt','test'||n,'word','test'||n,'meaning','뜻'||n,'example','test example','translation','예문','mode','engToKor','correctId','correct'||n,'options',jsonb_build_array(jsonb_build_object('id','correct'||n,'text','뜻'||n),jsonb_build_object('id','wrong'||n,'text','오답')))) into qs from generate_series(0,count_questions-1)n;
 r:=public.battle_play(a,'invite',null,jsonb_build_object('target',b,'questions',qs,'settings',jsonb_build_object('title','검증','mode','mixed','count',count_questions)));room:=(r->>'id')::uuid;
 ...
 begin perform public.battle_play(a,'invite',null,jsonb_build_object('target',b,'questions',qs,'settings',jsonb_build_object('count',count_questions)));raise exception 'FAIL overlap';exception when others then if SQLERRM='FAIL overlap' then raise;end if;end;
 ...
 for i in 0..count_questions-1 loop
  ...
  if i<count_questions-1 then assert r->>'status'='playing','premature finish';end if;
 end loop;
 assert r->>'status'='finished';assert (r->>'winner')::uuid=a;assert jsonb_array_length(r->'review')=count_questions;
 r:=public.battle_play(b,'poll',room);assert not (r->'review'->0->>'correct')::boolean;
 end loop;
 -- Second room: timeout and forfeit.
 update public.word_battles set created_at=clock_timestamp()-interval '2 minutes' where host=a;
 r:=public.battle_play(a,'invite',null,jsonb_build_object('target',b,'questions',qs,'settings',jsonb_build_object('count',count_questions)));room:=(r->>'id')::uuid;
...
$$;
rollback;
select 'PASS: friendship, isolation, countdown, duplicate answers, reveal, 10/20/30 rounds, timeout, forfeit, block, grants. Fixtures rolled back.' as verification;
