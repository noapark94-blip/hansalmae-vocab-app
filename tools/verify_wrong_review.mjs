...
w.clearWrongSelection_();assert.equal($('hsmSelectedSave').disabled,true);
// Selection dock must not inherit the animated card's positioning context.
assert.equal($('hsmReviewDock'),null);
assert.equal($('wrongTestOpenButton').nextElementSibling.className,'wrong-selection-entry');
assert.equal($('wrongActiveFilter').textContent,'복습할 오답');
...
assert.equal($('hsmReviewMore').hidden,true);
d.querySelector('.wrong-batch-check').checked=true;w.updateWrongBulkToolbar_();
assert.equal($('hsmReviewMore').hidden,false);
assert.equal($('wrongSelectionModeButton').hidden,true);
assert.equal($('wrongTestOpenButton').textContent,'선택한 1개만 시험보기');
$('wrongSelectAllButton').click();assert.equal($('wrongSelectAllButton').textContent,'전체 해제');
$('wrongSelectAllButton').click();assert.equal($('wrongSelectAllButton').textContent,'전체 선택');
assert.equal($('wrongTestOpenButton').disabled,true);
d.querySelector('.wrong-batch-check').checked=true;w.updateWrongBulkToolbar_();
