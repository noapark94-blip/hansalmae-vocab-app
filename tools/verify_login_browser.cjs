...
await page.setViewportSize({width:393,height:852});await page.locator('#installAppButton').click();assert(await page.locator('#hsmInstallModal').isVisible());assert((await page.locator('#hsmInstallTitle').textContent()).includes(platform==='Android'?'안드로이드':'아이폰'));assert(!await page.locator('#hsmInstallNow').isVisible());
...
if(platform==='Android'){
 ...
 await page.locator('#installAppButton').click();assert(await page.locator('#hsmInstallNow').isVisible());await page.locator('#hsmInstallNow').click();assert.equal(await page.evaluate(()=>promptCalls),1);await page.evaluate(()=>dispatchEvent(new Event('appinstalled')));assert(!await page.locator('#installAppButton').isVisible());
}
await page.close();}
