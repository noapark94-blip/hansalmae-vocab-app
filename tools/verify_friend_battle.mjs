...
game={...game,status:'finished',reward:{points:15,xp:55,eligibleCount:30,reason:'awarded',totalBefore:145,totalXp:200,level:3,previousLevel:2,title:'단어수집가',nextLevelXp:300,progress:50},winner:'me',hostScore:10,hostMs:42000,review:[{word:'word',meaning:'뜻',example:'Example',translation:'예문',correct:false}]};await w.hsmOpenFriendBattle_();assert(d.querySelector('.fb-result').textContent.includes('멋진 승리'));assert(d.querySelector('.fb-result-stats').textContent.includes('/ 30'));assert(d.querySelector('.fb-review').textContent.includes('Example'));assert(d.querySelector('.fb-rewards').textContent.includes('+15P'));assert(d.querySelector('.fb-rewards').textContent.includes('+55EXP'));assert.equal(d.querySelector('[role="progressbar"]').getAttribute('aria-valuenow'),'50');await click('[data-action="exit"]');assert(d.getElementById('fbSearch'));
